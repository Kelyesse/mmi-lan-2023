...
<body>
        ...
        while ($donnees = $req->fetch()) {
            ...
            if ($conducteurId == $_SESSION['PlayerId']) {
                ...
                while ($passagersAttente = $req2->fetch()) {
                    ...
                    echo '<a class="moderation" href="accept_passenger.php?playerId=' . $passagersAttente[0] . '&covoitId=' . $idCovoit . '"><button>Accepter</button></a>';
                    echo '<a class="moderation" href="reject_passenger.php?playerId=' . $passagersAttente[0] . '&covoitId=' . $idCovoit . '"><button>Refuser</button></a>';
                    echo '</p>';
                }
    ...
    <div id="alertRetirerTrajet" hidden>
        <div>
            <h3>Êtes vous sur de vouloir vous retirer du trajet ?</h3>
            <div>
                <?php
    ...
    <div id="alertSupprimerTrajet" hidden>
        <div>
            <h3>Êtes vous sur de vouloir supprimer votre trajet ?</h3>
            <p><i style="color: red">Supprimer votre trajet revient à laisser vos éventuels passagers sans covoiturage
                    pour la MMI LAN !</i></p>
            <div>
                <?php
    ...
    <div id="alertRejoindreTrajet" hidden>
        <div>
            ...
            <form action="rejoindreCovoit.php?covoitId=<?php echo $_GET['rejoindreCovoitId'] ?>" method="post">
                ...
                <input type="text" name="matos"
                    placeholder="Matériel encombrant que je voudrais amener (ordinateur fixe ou autre)" />
                <input type="text" name="nomDiscord"
                    placeholder="Pseudo Discord pour que le conducteur vous contacte en cas de besoin (optionnel)" />
